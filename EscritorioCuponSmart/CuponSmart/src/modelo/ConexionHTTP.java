...
public class ConexionHTTP {
    ...
    public static CodigoHTTP preticionPUTimagen(String url, byte[] imagen) {
        CodigoHTTP respuesta = new CodigoHTTP();
        try {
            URL urlServicio = new URL(url);
            HttpURLConnection conexionHTTP = (HttpURLConnection) urlServicio.openConnection();
            ...
            respuesta.setCodigoRespuesta(codigoRespuesta);
            if (codigoRespuesta == HttpURLConnection.HTTP_OK) {
                respuesta.setContenido(convertirContenido(conexionHTTP.getInputStream())); // el conexionHttp es el que alberga la informacion de la consulta del endpoint por eso lo mandamos como InputStream. convertidor es el metodo de abajo
            } else {
                respuesta.setContenido("CODE ERROR: " + codigoRespuesta);
            }
        } catch (MalformedURLException e) {
